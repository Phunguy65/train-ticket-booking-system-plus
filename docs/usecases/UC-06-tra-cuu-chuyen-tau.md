...
| Mục                            | Nội dung                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| ------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Phụ thuộc                      | UC-05: Tra cứu ga tàu                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| Mục đích                       | Khách hàng muốn tìm chuyến tàu phù hợp để đặt vé. PM cung cấp ba cách tra cứu: tìm kiếm có bộ lọc (cursor-based), duyệt danh sách phân trang (offset-based) và xem chi tiết một chuyến tàu cụ thể, giúp khách hàng đánh giá chuyến tàu trước khi chuyển sang đặt vé.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
...
| Xử lý ngoại lệ                 | Tìm kiếm không có kết quả → Hệ thống trả về danh sách rỗng. <br> Chuyến tàu không tồn tại (xem chi tiết) → Hệ thống trả về lỗi `SCHEDULED_TRIP_NOT_FOUND`. <br> Cursor không hợp lệ (sai định dạng) → Hệ thống trả về lỗi `CURSOR_INVALID`. <br> Tham số không hợp lệ (phân trang, bộ lọc, giá trị sắp xếp) → Hệ thống trả về lỗi `VALIDATION_ERROR`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
...
| Tiêu chí      | Phép thử                                                                                                                                  | Kết quả mong đợi                                                                | Ghi chú                                                        |
| ------------- | ----------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------- | -------------------------------------------------------------- |
| Chuẩn hóa     | Rà soát thiết kế ScheduledTripController, SearchScheduledTripsUseCase, GetScheduledTripsUseCase, GetScheduledTripByIdUseCase, ScheduledTripSearchPort, ScheduledTripRepository | Tuân thủ Clean Architecture, quy ước đặt tên và hợp đồng                        | Walkthrough/inspection                                         |
| Testability   | Rà soát khả năng mock ScheduledTripSearchPort, ScheduledTripRepository, CursorCodec trong unit test                                        | Có thể kiểm thử từng UseCase độc lập không cần DB thật                          | SearchPort và Repository là port, CursorCodec là utility thuần |
| Modularity    | Rà soát ranh giới trách nhiệm: Controller chỉ validate + route, UseCase chỉ orchestrate + cache, Repository/Port chỉ persistence/query    | Không trùng lặp trách nhiệm, coupling thấp                                      | Kiểm tra không có logic nghiệp vụ trong Controller             |
| Cache design  | Rà soát chiến lược cache: key naming, TTL, eviction policy cho 3 cache (scheduledTripFilter, scheduledTripList, scheduledTripById)          | Cache key deterministic, failure không được cache, invalidation khi data thay đổi | Kiểm tra cache key không collision giữa các bộ lọc khác nhau  |
...
| Truy cập công khai   | Gửi request không có access token                                                                           | 200 OK với kết quả bình thường                                                                                                                                                                                                                                     | Endpoint đã chuyển sang permitAll                     |
| Hiệu năng             | Benchmark 3 endpoint với 200 concurrent requests, bộ lọc phức tạp (nhiều điều kiện JOIN)                     | Response time p95 < 500ms trong điều kiện tải bình thường; cache giảm latency ≥ 50% so với lần gọi đầu                                                                                                                                                             | Ghi rõ môi trường test                               |
| Bảo mật               | Kiểm tra không lộ thông tin nhạy cảm trong response, SQL injection test                                      | Response chỉ chứa fields công khai; parameterized query chống injection                                                                                                                                                                                            | Kiểm tra cả cursor decode không bị injection         |
...
|---|---------------|--------|---------------|-----------------|-------------------|-----------|
| 1 | `filterReturnsEmptyMessageWhenNoTripsMatch` | Filter không có kết quả trả message rỗng | `GET /api/v1/scheduled-trips:filter` | `scheduled_trips` | `200` + SliceResponse rỗng + message | `backend/src/test/java/.../train/infrastructure/web/ScheduledTripControllerTest.java:38` |
| 2 | `filterOmitsMessageWhenTripsExist` | Filter có kết quả không có message | `GET /api/v1/scheduled-trips:filter` | `scheduled_trips, route_templates, trains, stations` | `200` + `SliceResponse<SearchScheduledTripsResponse>` | `backend/src/test/java/.../train/infrastructure/web/ScheduledTripControllerTest.java:56` |
| 3 | `list_returns200WithPagedScheduledTrips` | Danh sách chuyến tàu phân trang thành công | `GET /api/v1/scheduled-trips` | `scheduled_trips` | `200` + `PageResponse<ScheduledTripResponse>` | `backend/src/test/java/.../train/infrastructure/web/ScheduledTripControllerListAndGetByIdTest.java:55` |
| 4 | `list_returns200WithEmptyPageWhenNoScheduledTrips` | Danh sách rỗng | `GET /api/v1/scheduled-trips` | `scheduled_trips` | `200` + page rỗng | `backend/src/test/java/.../train/infrastructure/web/ScheduledTripControllerListAndGetByIdTest.java:72` |
| 5 | `list_usesDefaultPaginationWhenNoParamsProvided` | Phân trang mặc định | `GET /api/v1/scheduled-trips` | `scheduled_trips` | Gọi UseCase với page=0, size=20 | `backend/src/test/java/.../train/infrastructure/web/ScheduledTripControllerListAndGetByIdTest.java:85` |
| 6 | `getById_returns200WithScheduledTripDetailResponseWhenFound` | Xem chi tiết thành công | `GET /api/v1/scheduled-trips/{id}` | `scheduled_trips, route_templates, trains, stations` | `200` + `ScheduledTripDetailResponse` | `backend/src/test/java/.../train/infrastructure/web/ScheduledTripControllerListAndGetByIdTest.java:101` |
| 7 | `getById_returns404WhenScheduledTripNotFound` | Xem chi tiết không tồn tại | `GET /api/v1/scheduled-trips/{id}` | `scheduled_trips` | `404` + `SCHEDULED_TRIP_NOT_FOUND` | `backend/src/test/java/.../train/infrastructure/web/ScheduledTripControllerListAndGetByIdTest.java` |
| 8 | `execute_returnsPageResponseWithMappedScheduledTrips` | UseCase GetScheduledTrips map đúng fields | `GET /api/v1/scheduled-trips` | `scheduled_trips` | `PageResponse<ScheduledTripResponse>` | `backend/src/test/java/.../train/application/usecase/GetScheduledTripsUseCaseTest.java:44` |
| 9 | `execute_returnsEmptyPageWhenNoScheduledTripsExist` | UseCase GetScheduledTrips trả trang rỗng | `GET /api/v1/scheduled-trips` | `scheduled_trips` | `PageResponse` rỗng | `backend/src/test/java/.../train/application/usecase/GetScheduledTripsUseCaseTest.java:66` |
| 10 | `execute_delegatesToRepositoryWithCorrectSortOrder` | UseCase truyền sort đúng | `GET /api/v1/scheduled-trips` | `scheduled_trips` | Sort = [departureTime ASC, id ASC] | `backend/src/test/java/.../train/application/usecase/GetScheduledTripsUseCaseTest.java:85` |
| 11 | `execute_preservesPaginationMetadata` | UseCase giữ nguyên metadata phân trang | `GET /api/v1/scheduled-trips` | `scheduled_trips` | page, size, hasNext, total đúng | `backend/src/test/java/.../train/application/usecase/GetScheduledTripsUseCaseTest.java:118` |
| 12 | `execute_returnsScheduledTripDetailResponseWhenFound` | UseCase GetById trả response khi tìm thấy | `GET /api/v1/scheduled-trips/{id}` | `scheduled_trips, route_templates, trains, stations` | `Result.success(ScheduledTripDetailResponse)` | `backend/src/test/java/.../train/application/usecase/GetScheduledTripByIdUseCaseTest.java:43` |
| 13 | `execute_returnsScheduledTripNotFoundWhenRepositoryReturnsEmpty` | UseCase GetById trả lỗi khi không tìm thấy | `GET /api/v1/scheduled-trips/{id}` | `scheduled_trips` | `Result.failure(ScheduledTripNotFound)` | `backend/src/test/java/.../train/application/usecase/GetScheduledTripByIdUseCaseTest.java:81` |
| 14 | `execute_mapsAllFieldsFromEnrichedSummaryToResponse` | UseCase GetById map đầy đủ fields | `GET /api/v1/scheduled-trips/{id}` | `scheduled_trips, route_templates, trains, stations` | Tất cả fields mapped chính xác | `backend/src/test/java/.../train/application/usecase/GetScheduledTripByIdUseCaseTest.java:141` |
| 15 | `filter_passesXssPayloadThroughWithoutSanitization` | Pen-test XSS payload qua filter | `GET /api/v1/scheduled-trips:filter` | `scheduled_trips` | Response chứa payload nguyên bản | `backend/src/test/java/.../train/infrastructure/web/ScheduledTripControllerSecurityTest.java:51` |
| 16 | `filter_handlesSqlInjectionPayloadInCursorSafely` | Pen-test SQL injection qua cursor | `GET /api/v1/scheduled-trips:filter` | `scheduled_trips` | Parameterized query chống injection | `backend/src/test/java/.../train/infrastructure/web/ScheduledTripControllerSecurityTest.java:72` |
| 17 | `getById_returns404ForNonExistentUuid` | Pen-test UUID không tồn tại | `GET /api/v1/scheduled-trips/{id}` | `scheduled_trips` | `404` | `backend/src/test/java/.../train/infrastructure/web/ScheduledTripControllerSecurityTest.java:105` |
| 18 | `filter_handles50ConcurrentRequestsWithSameQuery` | Stress test 50 concurrent filter requests | `GET /api/v1/scheduled-trips:filter` | `scheduled_trips, route_templates, trains, stations` | 50 kết quả nhất quán, cache hoạt động | `backend/src/test/java/.../train/application/usecase/SearchScheduledTripsStressTest.java:65` |
| 19 | `getById_handles50ConcurrentRequestsForSameScheduledTrip` | Stress test 50 concurrent getById requests | `GET /api/v1/scheduled-trips/{id}` | `scheduled_trips, route_templates, trains, stations` | 50 kết quả nhất quán | `backend/src/test/java/.../train/application/usecase/SearchScheduledTripsStressTest.java:106` |

### Frontend

| # | Tên test case | Mô tả | Component / Hook | Kết quả mong đợi | File test |
|---|---------------|--------|------------------|-------------------|-----------|
| 1 | `renders origin, destination, date fields and search button` | Form tìm chuyến hiển thị đúng các trường | `TripSearchForm` | Render đúng labels và button | `frontend/customer/src/components/search/trip-search-form.test.tsx:34` |
...
| 4 | `validates search form with all required fields` | Integration test validate form tìm chuyến | `tripSearchSchema` | Parse thành công | `frontend/customer/src/__tests__/customer-flows.integration.test.ts:31` |
| 5 | `rejects search when origin and destination are the same` | Từ chối khi ga đi = ga đến | `tripSearchSchema` | Parse thất bại | `frontend/customer/src/__tests__/customer-flows.integration.test.ts:42` |
| 6 | `rejects search when departure date is in the past` | Từ chối ngày khởi hành trong quá khứ | `tripSearchSchema` | Parse thất bại | `frontend/customer/src/__tests__/customer-flows.integration.test.ts:60` |
| 7 | `serializes search params to URLSearchParams and parses back` | Serialize/parse search params cho trang kết quả | `serializeTripSearchParams` | Round-trip chính xác | `frontend/customer/src/__tests__/customer-flows.integration.test.ts:71` |
| 8 | `complete search to booking params flow` | E2E flow: search → chọn chuyến → chọn ghế → booking | Integration | Toàn bộ flow hoạt động đúng | `frontend/customer/src/__tests__/customer-flows.integration.test.ts:241` |
...
| ----------------------------------- | ---------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------- |
| Tìm kiếm có bộ lọc (cursor-based)  | Đúng nhu cầu: khách hàng lọc chuyến tàu theo ga đi/đến, ngày, trạng thái, giá, ghế trống             | Luồng xử lý chuẩn hóa qua Controller→UseCase→SearchPort, cursor encode/decode tách biệt, cache deterministic  | Unit test UseCase (happy, empty, bad cursor, bad params), integration test endpoint, verify cache hit/miss            |
| Duyệt danh sách (offset-based)      | Đúng nhu cầu: khách hàng duyệt toàn bộ chuyến tàu phân trang                                         | UseCase đơn giản delegate xuống Repository, cache theo page+size                                               | Unit test UseCase (happy, bad pagination), integration test endpoint, verify sắp xếp departureTime ASC               |
| Xem chi tiết chuyến tàu             | Đúng nhu cầu: khách hàng xem đầy đủ thông tin chuyến tàu kèm tàu, tuyến đường, ga                    | UseCase truy vấn enriched summary, cache trừ failure, trả Result monad                                         | Unit test UseCase (found, not found), integration test endpoint, verify response chứa đủ nested objects              |
| Truy cập công khai             | Endpoint công khai, không yêu cầu xác thực                                                         | Spring Security permitAll cho GET endpoints                                                    | Test 200 OK khi không có token trên cả 3 endpoint                                                                     |
| Caching                             | Giảm tải DB cho thao tác chỉ đọc, cải thiện thời gian phản hồi                                        | 3 cache riêng biệt (filter, list, byId), key naming convention rõ ràng, failure không cache                     | Verify cache eviction khi data thay đổi, verify cache key không collision, benchmark latency có/không cache          |
