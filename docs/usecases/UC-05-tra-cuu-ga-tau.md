...
| Xử lý ngoại lệ                 | Tìm kiếm không có kết quả → Hệ thống trả về danh sách rỗng kèm thông báo "No stations matched your search.". <br> Ga không tồn tại (xem chi tiết) → Hệ thống trả về lỗi `STATION_NOT_FOUND`. <br> Tham số phân trang không hợp lệ → Hệ thống trả về lỗi `VALIDATION_ERROR`.                                                                                                                                                                                                                                                                                                                                                                                   |
...
| Tiêu chí          | Phép thử                                                                                  | Kết quả mong đợi                                                    | Ghi chú                                    |
| ----------------- | ----------------------------------------------------------------------------------------- | ------------------------------------------------------------------- | ------------------------------------------ |
| Xử lý chính xác   | Test luồng chính (search có kết quả, list phân trang, detail thành công), luồng lỗi (không tìm thấy ga, validation fail) | 200 + đúng response format cho mỗi endpoint; 404 + STATION_NOT_FOUND; 400 + VALIDATION_ERROR | Kết hợp unit test UseCase + integration test endpoint |
...
| Truy cập công khai | Kiểm tra endpoint không yêu cầu xác thực, truy cập công khai                  | 200 OK khi không có token; response chỉ chứa thông tin công khai              | Endpoint đã chuyển sang permitAll          |
